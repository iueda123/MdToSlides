...
::: {.layout-split style="display:grid; grid-template-rows:30% 70%; height:70vh;"}
![Sample Figre](img/figure-top.svg){width=90%; style="justify-self:center; align-self:end;"}

::: {.split-caption style="padding:0 1rem 1rem;"}
- CSS グリッドで上下2段を作り、上段に図、下段に説明文
- `grid-template-rows:30% 70%` は上下2段の高さ配分（上段30%、下段70%）を指定
- `height:70vh` は表示領域の高さの 70% をコンテナ全体の高さにする指定
...
- 上段に文章、下段に図を配置して視線の流れを整理
- 下段の図は右寄せにし、中央の間延びを防ぐ
- `justify-self:end` と `align-self:end` で右下に揃える

::: {.floating-figure style="justify-self:end; align-self:end; width:30%;"}
